...
class aceinna_test_case():
    ...
    def init_test_dict(self): # ['item', 'function', 'target']
        ...
        self.test_case.append(['4.2.7', 'set_unit_behavior', 'self.test_file.write([item, self.set_unit_behavior(targetdata), self.function_measure_data[key]])', [00, 66]])
        ...
        self.test_case.append(['5.1.5', 'set_unit_behavior', 'self.test_file.write([item, self.set_unit_behavior(targetdata, saved_rst=True), self.function_measure_data[key]])', [00, 66]])
        ...
        self.test_case.append(['', '', 'self.test_file.write([item])', ''])
        self.test_case.append(['5.2.1', 'set_pkt_rate', 'self.test_file.write([item, self.set_pkt_rate(targetdata, nosaved_rst=True), self.function_measure_data[key]])', '0x05'])
        self.test_case.append(['5.2.2', 'set_pkt_type', 'self.test_file.write([item, self.set_pkt_type(targetdata, nosaved_rst=True), self.function_measure_data[key]])', '0x0F'])
        self.test_case.append(['5.2.3', 'set_lpf_filter', 'self.test_file.write([item, self.set_lpf_filter(targetdata, nosaved_rst=True), self.function_measure_data[key]])', '0x0505'])
        self.test_case.append(['5.2.4', 'set_orientation', 'self.test_file.write([item, self.set_orientation(targetdata, nosaved_rst=True), self.function_measure_data[key]])', '0x0009'])
    ...
    def test_hw_bit(self, target_data): # 4.1.7
        if self.debug: eval('print(k, i)', {'k':sys._getframe().f_code.co_name,'i':target_data})
        measure_data = self.dev.request_cmd('hw_bit')
        if measure_data == False: 
            self.function_measure_data[sys._getframe().f_code.co_name] = measure_data
            return measure_data
        self.function_measure_data[sys._getframe().f_code.co_name] = measure_data  
        return int(measure_data, 16) == int(target_data, 16)
    ...
    def set_pkt_type(self, target_data, saved_rst = False, nosaved_rst = False, type_measure = False): # 4.2.4 4.3.1-4.3.4 5.1.2 5.2.2
        ...
        if int(target_data, 16) < 256:
            ...
            self.dev.set_cmd('set_pkt_type', [int(target_data, 16)&0xff, (int(target_data, 16)>>8)&0xff])
        time.sleep(0.2)
        if saved_rst == True:
            self.dev.set_cmd('save_config', [2]) # save and restart
            time.sleep(2)
    ...
    def set_bank_ps1(self, target_data, saved_rst = False): # 4.2.8
        ...
        if self.dev.type_name == 'MTLT335':
            data = [pkt_rate, pkt_type, dig_filter, ori_request, unit_bhr, algo_ctl, 0]
        if self.dev.type_name == 'OPENIMU300RI':
            ...
            try:
                for j in try_idx:
                    ...
                    if item_dict.get('type') == 'set': # set cmd
                        ...
                        feedback2 = self.dev.new_request_cmd(src = 0, new_pgn = 0xFF00 + newps, reversed=True)
                        if (feedback != False) or (feedback2 == False):
                            results[cmd_name] = False
                        else:
                            results[cmd_name] = True
    ...
    def get_msg(self, target_data='acc_hr', back_default = True): # 4.3.6
        if self.debug: eval('print(k, i)', {'k':sys._getframe().f_code.co_name,'i':target_data})
        value = pow(2, len(self.dev.predefine.get('types_name'))) - 1
        if value > 255:
            self.dev.set_cmd('set_pkt_type', [value&0xFF, (value>>8)&0xFF])
        else:
    ...
    def try_type_list(self, target_data, actual_measure = True): # 5.4.1-5.4.18
        ...
        types_data = len(self.dev.predefine.get('types_name'))
        if types_data == 9:
            type_list          = [0, 1, 2, 4, 7, 8, 0xB, 0xF, 0xE, 0x13, 0x16, 0x17, 0x1B, 0x1C, 0x1E, 0x1F, 0x5F, 0xDF, 0x1DF]
        if types_data == 6:
    ...
    def try_unit_bhr_list(self, target_data):  # 5.8.1-5.8.21
        ...
        for idx, value in enumerate(enable_list):
            ...
            time.sleep(1)
            # input('123')            
            payload = self.dev.request_cmd('unit_behavior') if value < 2 else self.dev.request_cmd('unit_behavior', reversed=False)
            # input('123')
            if payload == False: 
                bhr_set_ok = False
            else:
