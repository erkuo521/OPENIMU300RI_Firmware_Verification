...
class aceinna_device(): 
    def __init__(self, source_address, attribute_json, debug_mode = False, power_gpio = None, devtype=None, app_type=None):
        # self.power_pin = pwr_pin # positve of power pin 
        self.auto_power = power_gpio
        self.debug = debug_mode
        self.dev_type = devtype
        self.dev_app = app_type
    ...
    def request_cmd(self, cmd_name, reversed=True):
        '''
        ...
        if len(pgn_des):
            ...
            for i in range(3): # send 3 times to avoid not working in some abnormal condition
                ...
                if self.req_feedback_payload[cmd_idx] != None:                    
                    return self.prehandle_payload(self.req_feedback_payload[cmd_idx]['payload'], pgn_des)
                else:
                    pass # to be added                 
            return False 
        else:
    ...
    def prehandle_payload(self, payload, pgndes):
        if self.debug: eval('print(k, i)', {'k':sys._getframe().f_code.co_name,'i':[payload, pgndes]})
        key_list = list(pgndes.keys())
        if ('start_lth' not in key_list) or ('bit_mask' not in key_list):
            return payload
        strt_lth = pgndes.get('start_lth')
        bit_mask = pgndes.get('bit_mask')
        cmd_name = pgndes.get('name')
        temp_pl = payload[strt_lth[0]:sum(strt_lth)]
        if cmd_name == 'hw_bit':
            rlt_pl = hex(struct.unpack('<h', bytes.fromhex(temp_pl))[0])
            return hex(int(rlt_pl,16) & bit_mask)
        else:
            return temp_pl        
    ...
    def set_cmd(self, cmd_name, payload_without_src):
        ...
        '''
        if self.debug: eval('print(k, i)', {'k':sys._getframe().f_code.co_name,'i':[cmd_name, payload_without_src, self.sw_rst_support, self.auto_power.enabled]})
        if len([x for x in self.can_attribute if x['type'] == 'set' and x['name'] == cmd_name]) == 0:
            return False
        ...
        if self.dev_type != 'OPENIMU300RI':
            ...
            for i in range(3):   # some times unit will no feedback for 80FF59 Request, need to restart by SW or manualy restart in below
                if payload == False: 
                    ...
                    payload = self.request_cmd('unit_behavior')
            while payload == False:
                if self.debug: eval('print(k, i)', {'k':sys._getframe().f_code.co_name, 'i':'in while loop, pls note'})
                if self.auto_power.enabled: # only if enabled, it will power on and off by gpio automaticaly.  default will not use auto-power, need manual power on and off
                    self.auto_power.power_off()
                    time.sleep(4)
                    self.auto_power.power_on()
